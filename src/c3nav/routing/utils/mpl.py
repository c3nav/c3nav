...
from shapely.geometry import MultiPolygon, Polygon, GeometryCollection, polygon
...
class MplMultipolygonPath(MplPathProxy):
    ...
    def contains_points(self, points):
        ...
        for polygon in self.polygons:
            ix = np.argwhere(result == False).flatten()
...
class MplPolygonPath(MplPathProxy):
    ...
    def contains_points(self, points):
        # return self.exterior.contains_points(points)
        result = self.exterior.contains_points(points)
        for interior in self.interiors:
            if not result.any():
                break
            ix = np.argwhere(result == True).flatten()
