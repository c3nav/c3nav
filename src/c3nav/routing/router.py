...
from c3nav.mapdata.models.locations import DefinedLocation, LocationAncestry
    ...
    def rebuild(cls, update: MapUpdateTuple):
        ...
        for level in levels_query:
            ...
            for location in level.locations.all():
                definedlocations.setdefault(location.pk, RouterLocation(location)).targets.append(routerlevel)
...
@dataclass
class BaseRouterGeometryTarget[TargetT: GeometryMixin, GeometryT: BaseGeometry](BaseRouterDatabaseTarget[TargetT]):
    geometry: GeometryT = field(init=False)

    def __post_init__(self, src):
        super().__post_init__(src)
...
@dataclass
class RouterArea(BaseRouterGeometryTarget[Area, Polygon]):
    space_id: int = field(init=False)
    slow_down_factor: Decimal = field(init=False)

    def __post_init__(self, src: Area):
        super().__post_init__(src)
        self.space_id = src.space_id
        self.slow_down_factor = src.slow_down_factor
...
@dataclass
class RouterPOI(BaseRouterGeometryTarget[POI, Point]):
    space_id: int = field(init=False)
    x: float = field(init=False)
    y: float = field(init=False)
    altitude: float | None = None

    def __post_init__(self, src: POI):
        super().__post_init__(src)
        self.space_id = src.space_id
        self.x = src.x
        self.y = src.y
...
@dataclass
class RouterCoordinates(BaseRouterTarget[CustomLocation]):
    x: float = field(init=False)
    y: float = field(init=False)
    altitude: float | None = None

    def __post_init__(self, src: CustomLocation):
        super().__post_init__(src)
        self.x = src.x
        self.y = src.y
